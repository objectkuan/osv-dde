...
public:
    ...
    static blk *current_disk();

    explicit blk(pci::device& dev);
    virtual ~blk();

    void add_disk(void *gd);
    ...
    void set_capacity(unsigned long size) { _config.capacity = size; }
    struct request *get_request() { return _req_head; }
    void dequeue_request(struct request *req);
    void finish_request(struct request *req);

private:
    void cleanup_request(void);
    ...
    void *_gendisk;
    struct request *_req_head;

    spinlock _cleanup_lock;
    struct request *_finished_req_head;
